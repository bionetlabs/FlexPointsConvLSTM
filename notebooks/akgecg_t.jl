...
begin
    lseg = map(x -> length(x), values(vtfp["(T"])) |> sum
    nseg = map(x -> length(x), values(vtfp["(N"])) |> sum
    lseg, nseg
end

# ╔═╡ 9ac7039e-bac5-4762-babf-c6ae5be1ceae
begin
	lcf = map(x -> mean(map(y -> y.cf, x) |> collect), values(vtfp["(T"])) |> mean
	lprd = map(x -> mean(map(y -> y.prd, x) |> collect), values(vtfp["(T"])) |> mean
	lqs = map(x -> mean(map(y -> y.qs, x) |> collect), values(vtfp["(T"])) |> mean
...
let
    ...
    ax = Axis(
        fig1[1, 1],
        titlealign=:left,
        title=string("F) ", auxlabeldescribed["(T"]),
    ...
    save(joinpath(projectdir, "img", "Fig1_(T.png"), fig1, px_per_unit=1)
...
let
    vtlengths = map(vtfp["(T"]["I"]) do chunk
        chunk.data |> length
    end
...
begin
    ...
    lstmmodel1 = @load(NeuralNetworkClassifier, pkg = "MLJFlux", verbosity = 0)(
        builder=builder, epochs=500, batch_size=128, acceleration=CUDALibs(), optimiser=Flux.Optimisers.Adam(0.0015), rng=rngseed
    )
end
...
let
    ...
    accuracy_raw, precision_raw, recall_raw, f1_raw = classificationscore(y_test, yhat_raw, "(T")

    yhat_raw_fold = MLJ.predict_mode(rawmach1_fold, x_test_fold)
    accuracy_raw_fold, precision_raw_fold, recall_raw_fold, f1_raw_fold = classificationscore(y_test_fold, yhat_raw_fold, "(T")
    ...
    accuracy_reconstructed, precision_reconstructed, recall_reconstructed, f1_reconstructed = classificationscore(yr_test, yhat_reconstructed, "(T")
    ...
    accuracy_reconstructed_fold, precision_reconstructed_fold, recall_reconstructed_fold, f1_reconstructed_fold = classificationscore(
        yr_test_fold, yhat_reconstructed_fold, "(T"
    )
