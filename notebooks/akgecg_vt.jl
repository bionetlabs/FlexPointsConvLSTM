...
begin
	lseg = map(x -> length(x), values(vtfp["(VT"])) |> sum
...
begin
	lcf = map(x -> mean(map(y -> y.cf, x) |> collect), values(vtfp["(VT"])) |> mean
	lprd = map(x -> mean(map(y -> y.prd, x) |> collect), values(vtfp["(VT"])) |> mean
	lqs = map(x -> mean(map(y -> y.qs, x) |> collect), values(vtfp["(VT"])) |> mean
...
let
    ...
    ax = Axis(
        fig1[1, 1],
        titlealign=:left,
        title=string("B) ", auxlabeldescribed["(VT"]),
...
let
    vtlengths = map(vtfp["(VT"]["I"]) do chunk
        chunk.data |> length
    end
...
let
    ...
    accuracy_raw, precision_raw, recall_raw, f1_raw = classificationscore(y_test, yhat_raw, "(VT")

    yhat_raw_fold = MLJ.predict_mode(rawmach1_fold, x_test_fold)
    accuracy_raw_fold, precision_raw_fold, recall_raw_fold, f1_raw_fold = classificationscore(y_test_fold, yhat_raw_fold, "(VT")
    ...
    accuracy_reconstructed, precision_reconstructed, recall_reconstructed, f1_reconstructed = classificationscore(yr_test, yhat_reconstructed, "(VT")
    ...
    accuracy_reconstructed_fold, precision_reconstructed_fold, recall_reconstructed_fold, f1_reconstructed_fold = classificationscore(
        yr_test_fold, yhat_reconstructed_fold, "(VT"
    )
