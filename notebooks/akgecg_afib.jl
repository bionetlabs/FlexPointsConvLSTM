...
parameters = FlexPointsParameters(
    ...
    yresolution=0.005, # 0.0075, # values with smaller Δ are considered as one point 
    polyapprox=1,
...
# ╔═╡ d5eec9ec-e7ce-49c2-94ec-02f39a0b95c2
begin
	lseg = map(x -> length(x), values(vtfp["(AFIB"])) |> sum
	nseg = map(x -> length(x), values(vtfp["(N"])) |> sum
	lseg, nseg
end

# ╔═╡ 2dcba350-e468-458d-b459-eecd0433e4f2
begin
	lcf = map(x -> mean(map(y -> y.cf, x) |> collect), values(vtfp["(AFIB"])) |> mean
	lprd = map(x -> mean(map(y -> y.prd, x) |> collect), values(vtfp["(AFIB"])) |> mean
	lqs = map(x -> mean(map(y -> y.qs, x) |> collect), values(vtfp["(AFIB"])) |> mean
...
let
    ...
    ax = Axis(
        fig1[1, 1],
        titlealign=:left,
        title=string("D) ", auxlabeldescribed["(AFIB"]),
    ...
    save(joinpath(projectdir, "img", "Fig1_(AFIB.png"), fig1, px_per_unit=1)
...
let
    vtlengths = map(vtfp["(AFIB"]["I"]) do chunk
        chunk.data |> length
    end
...
let
    ...
    accuracy_raw, precision_raw, recall_raw, f1_raw = classificationscore(y_test, yhat_raw, "(AFIB")

    yhat_raw_fold = MLJ.predict_mode(rawmach1_fold, x_test_fold)
    accuracy_raw_fold, precision_raw_fold, recall_raw_fold, f1_raw_fold = classificationscore(y_test_fold, yhat_raw_fold, "(AFIB")
    ...
    accuracy_reconstructed, precision_reconstructed, recall_reconstructed, f1_reconstructed = classificationscore(yr_test, yhat_reconstructed, "(AFIB")
    ...
    accuracy_reconstructed_fold, precision_reconstructed_fold, recall_reconstructed_fold, f1_reconstructed_fold = classificationscore(
        yr_test_fold, yhat_reconstructed_fold, "(AFIB"
    )
